...
case class Params(
                   input: String = "src/main/resources/docs/data.txt",
                   k: Int = 3,
                   maxIterations: Int = 10,
                   docConcentration: Double = -1,
                   topicConcentration: Double = -1,
                   vocabSize: Int = 2900000,
                   stopwordFile: String = "src/main/resources/docs/stopWords.txt",
                   algorithm: String = "em",
  ...
  def getLemmaText(document: String, morphology: Morphology) = {
    ...
    val value = new Document(document).sentences().toList.flatMap { a =>
      ...
      (words zip tags).toMap.map { a =>
        ...
        val addedWoed = if (newWord.length > 4) {
          newWord
        } else {
          ""
        }
